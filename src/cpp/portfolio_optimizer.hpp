            ...
            double ann_days = 252.0) const noexcept
        {
            ...
            for (int t = 21; t < T; ++t)
            {
                ...
                const double raw_pnl = t + 1 < T
                                           ? w.dot(returns.row(t + 1).transpose())
                                           : 0.0;
            ...
            kpis.sortino_ratio = dd_vol > 1e-12
                                     ? mu_pnl / dd_vol * std::sqrt(ann_days)
                                     : 0.0;
            ...
            kpis.gross_alpha_bps = (kpis.cagr + total_tc_cost / (T / ann_days)) * 10000.0;
            kpis.net_alpha_bps = kpis.cagr * 10000.0;

            // ── Decay monitoring ───────────────────────────────────────────────
            kpis.ic_half_life_days = half_life_decay(ic_valid);
            kpis.decay_alert = kpis.ic_half_life_days < 30.0 || kpis.mean_ic < 0.02;
