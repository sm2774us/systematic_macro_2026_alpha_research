...
namespace alpha::strategies
{
    ...
    public:
        ...
        [[nodiscard]] Output tick(const DayData &d) noexcept
        {
            ...
            if (delta_10y < 0 && slope_2s10s > 0)
                regime = 1; // bull-steep
            else if (delta_10y < 0 && slope_2s10s <= 0)
                regime = 0; // bull-flat
            else if (delta_10y >= 0 && slope_2s10s <= 0)
                regime = 2; // bear-flat
            // else bear-steep
            ...
            const double fso_raw = -d.swap_spread_30y * 1000.0 + d.cds_5y_proxy * 10000.0;
        ...
        [[nodiscard]] Output tick(const DayData &d) noexcept
        {
            ...
            for (int i = 0; i < N_INDICES; ++i)
            {
                ...
                const double erb_z = rev_trackers_[i].ready()
                                         ? rev_trackers_[i].z(d.eps_revisions[i])
                                         : 0.0;

                // Interaction term: ISM × ERB (the key non-replicable feature)
                const double interaction = ism_z * erb_z;

                double sig = 0.45 * erb_z + 0.35 * interaction + 0.20 * d.pead_signals[i];
                out.signals[i] = sig;
        ...
        [[nodiscard]] Output tick(const DayData &d) noexcept
        {
            ...
            for (int i = 0; i < N_ENERGY; ++i)
            {
                ...
                const double is_z = is_trackers_[i].ready()
                                        ? is_trackers_[i].z(d.inventory_surprises[i])
                                        : 0.0;
                ...
                const double sig = 0.4 * is_z + 0.4 * interaction + 0.2 * opec_decay;
                out.signals[i] = sig;
    ...
    public:
        ...
        {
            // Build FCI composite
            const double fci_raw = 0.40 * (-d.swap_spread_30y * 1000.0) + 0.35 * (d.cds_5y * 10000.0) + 0.25 * (d.tbill_spike * 10000.0);
            ...
            for (int i = 0; i < N_ASSETS; ++i)
            {
                ...
                const double prop_lag = fci_buf_.ready(static_cast<int>(KERNEL_LEN / 2))
                                            ? fci_buf_.convolve(kPropKernels[i])
                                            : 0.0;

                double sig = cfg_.weights(i, 0) * fci_z + cfg_.weights(i, 1) * prop_lag;

                // Threshold gate: signal dampened in quiet regimes
                if (std::abs(fci_z) < cfg_.fci_shock_threshold)
                    sig *= 0.3;
