...
static nb::object mat2np(const Eigen::MatrixXd &m)
{
    ...
    nb::capsule del(d, [](void *p) noexcept
        ...
        .def("compute_rrdm", [](alpha::pdrrm::PDRRMEngine &self, const NpArray2D &nom, const NpArray2D &be)
             {
                 ...
                 return mat2np(*r); }, "nominal_rates"_a, "breakevens"_a)
        .def("compute_pss", [](alpha::pdrrm::PDRRMEngine &self, const NpArray2D &s)
             {
                 auto r = self.compute_pss(np2mat(s));
                 if (!r) throw std::runtime_error(r.error().message);
                 return mat2np(*r); }, "cb_surprises"_a)
        .def("compute_rac", [](alpha::pdrrm::PDRRMEngine &self, const NpArray2D &fp, const NpArray2D &ret)
             {
                 auto r = self.compute_rac(np2mat(fp), np2mat(ret));
                 if (!r) throw std::runtime_error(r.error().message);
                 return mat2np(*r); }, "forward_premium"_a, "returns"_a)
        .def("update_weights", [](alpha::pdrrm::PDRRMEngine &self, const NpArray2D &X, const NpArray1D &y)
             {
                 auto r = self.update_weights(np2mat(X), np2vec(y));
                 if (!r) throw std::runtime_error(r.error().message);
                 return vec2np(*r); }, "X_is"_a, "y_is"_a);

    // BacktestEngine ------------------------------------------------------
    nb::class_<alpha::portfolio::BacktestEngine>(m, "BacktestEngine")
        .def(nb::init<>())
        .def("run", [](const alpha::portfolio::BacktestEngine &self, const NpArray2D &sig, const NpArray2D &ret, double tc, double ann)
             {
        ...
        .def("tick", [](alpha::strategies::TPMCREngine &self, const NpArray1D &data, const NpArray1D &rets)
             {
                 ...
                 return result; }, "data"_a, "returns"_a);
        ...
        .def("tick", [](alpha::strategies::FDSPEngine &self, const NpArray1D &macro, const NpArray1D &rets)
             {
                 ...
                 result["signals"]   = out.signals;
                 result["positions"] = out.positions;
                 return result; }, "macro"_a, "returns"_a);

    // Utility functions ---------------------------------------------------
    m.def("sharpe_ratio", [](const NpArray1D &pnl, double ann)
          { return alpha::math::sharpe_ratio(np2vec(pnl), ann); }, "pnl"_a, "ann_factor"_a = 252.0);

    m.def("max_drawdown", [](const NpArray1D &cum_pnl)
          { return alpha::math::max_drawdown(np2vec(cum_pnl)); }, "cum_pnl"_a);

    m.def("half_life_decay", [](const NpArray1D &series)
          { return alpha::math::half_life_decay(np2vec(series)); }, "series"_a);
}
